...
function App() {
    ...
    if (letterPos !== 5) {
      ...
      if(isWordInDictionary(word) && wordOfTheDay !== undefined){
        if(word === wordOfTheDay){
          ...
          const newBoard = [...board];
          const guessArray = newBoard[attempt].map(element => {return {key: element.value.toLowerCase(), state: 'grey'};});
  ...
  const keyBoardKeyClick = ({key}: any) => {
    ...
    if(keyValue !== 'Enter' && keyValue !== 'Delete' && keyValue.length > 1)  return;
    if(keyValue === ' ')  return;
